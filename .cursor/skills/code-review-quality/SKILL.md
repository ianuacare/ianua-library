...
description: Review Flask/Python backend code for correctness, maintainability, performance, and healthcare security. Use when reviewing PRs, refactors, or technical debt.
---

# Code Review Quality

Prioritize findings by impact: correctness and security first, then maintainability/performance.

## Review Checklist

### Correctness
- Validate endpoint behavior, status codes, and payload contracts
- Verify input validation on path/query/body parameters
- Check error handling and exception mapping consistency

### Architecture
- Thin Flask routes, business logic in services/modules
- No domain logic scattered across blueprints
- Clear adapter boundary for `ianuacare` integration

### Security (healthcare critical)
- No PII/clinical data in logs
- Auth and authorization checks present on protected endpoints
- No string-interpolated SQL or untrusted code execution
- Secrets never hardcoded

### Performance
- Avoid N+1 patterns / repeated expensive external calls
- Use caching, batching, or async processing when needed
- Ensure timeouts/retries/circuit breaking for upstream dependencies

### Test quality
- Tests cover happy path + relevant failures
- API tests assert status + stable response schema
- New behavior has meaningful regression protection

## Output format
For each finding:
```
[SEVERITY] path/to/file.py
Issue: what fails and why it matters
